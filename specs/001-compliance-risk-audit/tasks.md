...
- [ ] T019 [P] [US2] Implement scan-stage animation in `frontend/src/components/ScanStages.tsx`: indeterminate progress with actual elapsed time while the single POST is in flight; describe the workflow without claiming stage completion because this endpoint does not stream stage events
