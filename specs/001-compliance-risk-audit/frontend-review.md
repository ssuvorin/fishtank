...
5. **Integration:** backend must enable CORS for the actual frontend origin, return the contract's top-level revenue/coverage fields, and return all ten findings. Live scoring and synthesis remain unverified until the backend exists and is configured.
